  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="relative h-screen w-full overflow-hidden">
        ...
        <div className="absolute inset-0 bg-greenDark/40 flex items-center justify-center">
          <div className="text-center text-white p-6 max-w-4xl">
            <h2 className="text-sm md:text-base tracking-[0.5em] mb-4 uppercase text-greenJade">Metaphysics & Spirituality</h2>
            <h1 className="text-5xl md:text-7xl font-serif font-light mb-8 leading-tight">
              提升生活品質的<br/>玄學之選
            </h1>
            <p className="text-lg md:text-xl font-light mb-10 text-greenLight max-w-2xl mx-auto">
              杜乾彰師傅為您提供專業風水設計、玄學諮詢及傳統祭祀儀式，<br/>融合心理學與傳統智慧，助您創造順利和諧的人生。
            </p>
            <div className="flex flex-col md:flex-row gap-6 justify-center">
              <Link to="/membership">
                <Button variant="primary" className="bg-greenJade !text-white font-bold hover:bg-accent tracking-widest">
                  探索會員制度
                </Button>
              </Link>
              <Link to="/services">
                <Button variant="outline" className="border-2 border-greenJade text-greenJade hover:bg-greenJade hover:text-white tracking-widest">
                  預約服務
                </Button>
              </Link>
            </div>
        ...
        <div className="container mx-auto px-6">
          <div className="flex flex-col md:flex-row gap-16 items-center">
            ...
            <div className="w-full md:w-1/2">
              ...
              <h2 className="text-4xl font-serif mb-8 text-primary">杜乾彰師傅 <span className="text-xl block mt-2 text-secondary/70">Master Du Qianzhang</span></h2>
              <p className="text-secondary leading-loose mb-6 font-light text-justify">
                作為六壬伏英館及風火院傳教師傅，杜乾彰師傅不僅承襲了深厚的道教傳統，更具備國際認證催眠師及心理學專業資格。
              </p>
              <p className="text-secondary leading-loose mb-8 font-light text-justify">
              ...
              <div className="grid grid-cols-2 gap-8 mb-8">
                {PROFILE_STATS.map((stat, idx) => (
                  <div key={idx}>
                    <p className="text-3xl font-serif text-accent">{stat.value}<span className="text-sm ml-1 text-primary">{stat.suffix}</span></p>
                    <p className="text-xs text-secondary/70 uppercase tracking-wider mt-2">{stat.label}</p>
                  </div>
                ))}
              </div>

              <Link to="/about" className="group flex items-center text-sm tracking-widest border-b-2 border-accent pb-1 w-fit text-primary hover:text-accent">
      ...
      <section className="py-24 bg-secondary text-white relative">
        <div className="container mx-auto px-6 text-center max-w-3xl">
          <div className="flex justify-center mb-8">
            {[1,2,3,4,5].map(i => <Star key={i} className="w-5 h-5 text-greenJade fill-current" />)}
          </div>
          <p className="text-2xl md:text-3xl font-serif italic leading-relaxed mb-8 text-greenLight">
            "成為「全面貼心」會員後，我的事業和人際關係都有明顯改善。杜師傅的季度諮詢幫助我及時調整生活方向，讓我避開了許多潛在問題。"
          </p>
          <div className="text-sm tracking-widest uppercase text-greenJade">
            — 陳先生，金融業專業人士
          </div>
        </div>
      </section>
